...
class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
  ...
  public ErrorAttributes errorAttributes() {
    return new DefaultErrorAttributes() {
      ...
      public Map<String, Object> getErrorAttributes(
        ...
        ApiErrorResource error =
            new ApiErrorResource()
                .status(
                    defaultErrorResponse.get("status") != null
                        ? (Integer) defaultErrorResponse.get("status")
                        : 0)
                .message(
                    defaultErrorResponse.get("message") != null
                        ? defaultErrorResponse.get("message").toString()
                        : "");
  ...
  public ResponseEntity<ApiErrorResource> handleNotFoundException(NotFoundException exception) {
    ...
    return new ResponseEntity<>(
        new ApiErrorResource().status(HttpStatus.NOT_FOUND.value()).message(exception.getMessage()),
  ...
  // @see
  // [controlleradvice-not-triggered-for-jackson-errors](https://stackoverflow.com/questions/49242100/controlleradvice-not-triggered-for-jackson-errors)
